...
size_t CXMLArchive::SeekTo(size_t offset, int aSeek)
{
	DWORD newNode;
	xmlpp::Node *start;
...
UINT CXMLArchive::Read( void *buffer, size_t size, const TCHAR *tag )
{
	...
	UINT nLen = (unsigned int)attributeValue.size();
	std::string decodeString = base64_decode(attributeValue.data());

	if (size > decodeString.length())
	{
		size = decodeString.length();
	}

	memcpy(static_cast<LPBYTE>(buffer), decodeString.c_str(), size);
	return size;
}
...
{
	std::string value = base64_encode(static_cast<LPCBYTE>(buffer), (unsigned int)size);
	Write( value, tag );
}
