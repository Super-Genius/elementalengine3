...
*/
	...
	/// Load plugins that will hook and extend ELemental Engine
	/// \param searchPath = path + wildcard or path + filename to load
	/// \param dllPMap = reference to map of DLL hInstance to priorities
	/// \return int = error number 
	virtual int LoadPlugins(const TCHAR *searchPath, DLLPRIORITYMAP &dllPMap);

	/// Initialize the plugins that will hook and extend ELemental Engine
	/// \param dllPMap = reference to map of DLL hInstance to priorities
	/// \return int = error number 
	virtual int InitPlugins(DLLPRIORITYMAP &dllPMap);

	/// Unload plugins that are hooked and extend ELemental Engine
	/// \param dllPMap = reference to map of DLL hInstance to priorities
	/// \return int = error number 
	virtual int UnloadPlugins(DLLPRIORITYMAP &dllPMap);
	...
	{
		return m_PluginLoader->LoadPlugin(pathName, outInstance, priority);
	}
	...
	{
		return m_PluginLoader->UnloadPlugin(instance);
	}
	...
	{
		return m_PluginLoader->InitPlugin(instance);
	}
...
private:
	...
	IPluginLoader *m_PluginLoader;					/// plugin in loader pointer

	IFindFiles *m_FindFile;							/// platform dependent Find Files System
};
