...
// each platform defines there own Find files implementation
extern IFindFiles *GetFindFiles();
// each platform defines there own plugin loader
extern IPluginLoader *GetPluginLoader();
...
CToolBox::CToolBox()
{
	...
	m_PluginLoader = GetPluginLoader();

	// platform specific FindFile interface
	m_FindFile = GetFindFiles();
}
...
int CToolBox::LoadPlugins(const TCHAR *searchPath, DLLPRIORITYMAP &dllPMap)
{
	FIND_DATA fData;
	HANDLE hFile;
	BOOL done;
	...
	_tcscpy( szPath, searchPath );
	size_t iPathLength = _tcslen( szPath );

	for (size_t i=iPathLength; i > 0 && ((szPath[i] != '\\') && (szPath[i] != '/')); i--)
		wildIndex = i;
...
{
	DLLPRIORITYMAP::iterator dpmIter;

	for (dpmIter = dllPMap.begin(); dpmIter != dllPMap.end(); ++dpmIter)
	{
		if (m_PluginLoader->InitPlugin(dpmIter->second))
		{
			return GetLastError();
		}
	}
...
int CToolBox::UnloadPlugins(DLLPRIORITYMAP &dllPMap)
{
	DLLPRIORITYMAP::iterator dpmIter;

	for (dpmIter = dllPMap.begin(); dpmIter != dllPMap.end(); ++dpmIter)
	{
		if (m_PluginLoader->UnloadPlugin(dpmIter->second))
		{
			return GetLastError();
		}
	}
