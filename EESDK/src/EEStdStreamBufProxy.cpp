...
int CEEStdStreamBufProxy::underflow()
{
	...
	moveCnt = std::min(used, (unsigned int)m_Reserved);
	if (moveCnt > 0)
	{
	...
	size_t iCur = m_EEStream->Tell();
	m_EEStream->SeekTo( 0, SEEK_END);
	size_t iEnd = m_EEStream->Tell();
	m_EEStream->SeekTo( iCur, SEEK_SET);
	size_t iReadSize = (m_BufSize < (size_t)(iEnd-iCur)) ? m_BufSize : (size_t)(iEnd-iCur);
...
streamsize CEEStdStreamBufProxy::xsgetn(TCHAR *dest, streamsize sSize)
{
	...
	while (sSize)
	{
		...
		memcpy(dest + nRead, gptr(), (size_t)avail);
		gbump((int)avail);
...
pos_type CEEStdStreamBufProxy::seekoff(off_type offset, seekdir dir, openmode mode)
{
    ...
    pos_type pos = 
		m_EEStream->SeekTo((size_t)offset, 
