...
/// map of dll priorities to load/init
typedef map<DWORD, HINSTANCE> DLLPRIORITYMAP;
...
*/
	...
	virtual int LoadPlugins(const TCHAR *searchPath, DLLPRIORITYMAP &dllPMap) = 0;

	/// Initialize the plugins that will hook and extend ELemental Engine
	/// \param dllPMap = reference to map of DLL hInstance to priorities
	/// \return int = error number 
	virtual int InitPlugins(DLLPRIORITYMAP &dllPMap) = 0;

	/// Unload plugins that are hooked and extend ELemental Engine
	/// \param dllPMap = reference to map of DLL hInstance to priorities
	/// \return int = error number 
	virtual int UnloadPlugins(DLLPRIORITYMAP &dllPMap) = 0;
