...
// this freetype stuff is being moved from within CGLFont so it can be called at the right time by CFontManager
void InitFontImplementation()
{
	int error = FT_Init_FreeType( &EE_CGLFont::m_FontLibrary );
}

void DeInitFontImplementation()
{
	FACEMAP::iterator iter = EE_CGLFont::m_NormFaceMap.begin();
	while (iter != EE_CGLFont::m_NormFaceMap.end())
	{
		FT_Done_Face( iter->second.m_FTFace );
		iter->second.m_FTFace = NULL;
		iter++;
	}
	EE_CGLFont::m_NormFaceMap.clear();

	iter = EE_CGLFont::m_BoldFaceMap.begin();
	while (iter != EE_CGLFont::m_BoldFaceMap.end())
	{
		FT_Done_Face( iter->second.m_FTFace );
		iter->second.m_FTFace = NULL;
		iter++;
	}
	EE_CGLFont::m_BoldFaceMap.clear();

	iter = EE_CGLFont::m_ItalicFaceMap.begin();
	while (iter != EE_CGLFont::m_ItalicFaceMap.end())
	{
		FT_Done_Face( iter->second.m_FTFace );
		iter->second.m_FTFace = NULL;
		iter++;
	}
	EE_CGLFont::m_ItalicFaceMap.clear();

	iter = EE_CGLFont::m_BoldItalicFaceMap.begin();
	while (iter != EE_CGLFont::m_BoldItalicFaceMap.end())
	{
		FT_Done_Face( iter->second.m_FTFace );
		iter->second.m_FTFace = NULL;
		iter++;
	}
	EE_CGLFont::m_BoldItalicFaceMap.clear();

	FT_Done_FreeType( EE_CGLFont::m_FontLibrary );
	EE_CGLFont::m_FontLibrary = NULL;
}

//statics might need to move to a manager?
FT_Library						EE_CGLFont::m_FontLibrary;
FACEMAP							EE_CGLFont::m_NormFaceMap;
FACEMAP							EE_CGLFont::m_BoldFaceMap;
FACEMAP							EE_CGLFont::m_ItalicFaceMap;
FACEMAP							EE_CGLFont::m_BoldItalicFaceMap;
IMAGEMAP						EE_CGLFont::m_ImageMap;
...
bool EE_CGLFont::SetFont( const TCHAR * filename, int height )
{
...
bool EE_CGLFont::SetFontBold( const TCHAR * filename, int height )
{
...
bool EE_CGLFont::SetFontItalic( const TCHAR * filename, int height )
{
...
bool EE_CGLFont::SetFontBoldItalic( const TCHAR * filename, int height )
{
...
bool EE_CGLFont::SetFontFace(FONTTYPE type, const TCHAR* filename, int height)
{
			...
			else
			{
				...
				if( !error )
				{
					...
					pair<FACEMAP::iterator, bool> insResult = mapToCheck->insert(
						pair<DWORD, CGLFontFace>(hfile.GetUniqueID(), newface));
...
int EE_CGLFont::GetFontPixWidth( const TCHAR * str )
{
	if( m_Faces[NORMAL]  )
	{
...
int EE_CGLFont::GetFontPixWidth( CHashString fontFace, UINT fontsize, const TCHAR * str )
{
...
void EE_CGLFont::SetFontPixHeight( int height )
{
	if( m_Faces[NORMAL] )
	{
...
/// returns offset
int EE_CGLFont::UpdateBuffer( char ** pDest, int offset,IVertexBufferObject * vb )
{
	if( m_bVisible &&
		m_Faces[NORMAL] )
	{
...
int EE_CGLFont::UpdateIndexBuffer( char ** pDest, int offset,IIndexBuffer * ib )
{
	if( m_bVisible)
	{
...
void EE_CGLFont::SetText( const TCHAR * txt, char delimators[], int numtokendelim )
{
	PERFORMANCE_PROFILER_START(GLFont_ParseTags);
	m_TextWithTags = txt;
	unsigned int k;
	bool isBold = false;
	bool isItalic = false;
	...
	PERFORMANCE_PROFILER_STOP(GLFont_ParseTags);

	PERFORMANCE_PROFILER_START(GLFont_ParseColor);
	...
	PERFORMANCE_PROFILER_STOP(GLFont_ParseColor);

	PERFORMANCE_PROFILER_START(GLFont_SetParagraph);
	...
	PERFORMANCE_PROFILER_STOP(GLFont_SetParagraph);
...
int EE_CGLFont::GetCharNum(const TCHAR * txt, char delimators[], int numtokendelim, float width)
{
	static TCHAR defaultdelimator[] = {' '};
	if(	m_Faces[NORMAL] )
	{
