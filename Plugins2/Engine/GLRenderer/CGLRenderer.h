...
#define CGLRENDERER_DEFAULT_2DBUFFER_ALLOCATION 20000
...
struct CGLRenderer_ShaderConstant
{
	Vec4	vValue;
	bool	bChanged;

	CGLRenderer_ShaderConstant()
	{
...
typedef map< CHANNELDESCRIPTORLIST, GLenum,	VERTEXDESCRIPTIONCOMPARE > VERTEXDECLARATIONMAP;
typedef pair< CHANNELDESCRIPTORLIST, GLenum > VERTEXDECLARATIONMAP_PAIR;
...
public:
	...
	DWORD OnGetRendererInterface( DWORD size, void *params);
	...
	//GL specific
	GLenum GetVertexDescription( CHANNELDESCRIPTORLIST &channels, UINT streammode );
	...
	virtual void* GetAPIDevice( void ) { return NULL; }
	...
	void SetPixelShader( GLenum pshader );
	void SetVertexShader( GLenum vshader );
...
private:
	...
	CGLRenderer_ShaderConstant * m_VSConstants;
	CHANGEDSHADERCONSTANTSTACK m_PSConstantsChanged;
	CGLRenderer_ShaderConstant * m_PSConstants;
	
	//Redundancy and other tests. Might want to make these functions part of the interface
	void SetVertexStream( UINT stream, CGLVertexBufferObject * cvb );
	void SetIndices( CGLIndexBuffer * cib );
	...
	HRESULT SetGLRenderState( ENUMRENDERSTATE state, UINT value );
	HRESULT SetGLTextureStageState( UINT stage, ENUMTEXTURESTAGESTATE state, UINT value );
	HRESULT SetGLSamplerStageState( UINT stage, ENUMSAMPLERSTATE state, UINT value );
	...
	GLenum	m_BackBufferDepthSurface;
	...
	BYTE * m_IndexStream;
	GLenum	m_PixelShaderSet;
	GLenum	m_VertexShaderSet;
	GLenum	m_VertexDeclSet;
	DWORD	m_FVFSet;
	DWORD	m_APIGUID;
	...
	stack< Matrix4x4> m_MatrixStack[ 3 ];

	CGLRenderContext * m_RenderContext;
	...
	const static GLenum m_GLSamplerStateEnum[SAMPLERSTATE_COUNT];
	const static GLenum m_GLTextureStageStateEnum[TEXTURESTAGESTATE_COUNT];
	const static GLenum m_GLRenderStateEnum[RENDERSTATE_COUNT];
	const static UINT m_GLTextureStageParamEnum[TEXTURESTAGEPARAM_COUNT];
	const static UINT m_GLRenderStateParamEnum[RENDERSTATEPARAM_COUNT];
