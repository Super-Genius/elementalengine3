...
bool CVertexShader::Apply( IShaderDefines *pDefines )
{
	...
	CGLRend->SetVertexShader( pShader );
	return true;
}
...
{
	CGLRenderer * CGLRend = (CGLRenderer *)m_Renderer;	
	return CGLRend->SetVertexShaderConstantF( reg, data, num );
}
...
bool CVertexShader::SetConstant( DWORD reg, float x, float y, float z, float w )
{
	...
	data[2] = z;
	data[3] = w;
	return CGLRend->SetVertexShaderConstantF( reg, data, 1 );
}
...
bool CVertexShader::LoadVariant( IShaderDefines *pDefines, bool bCompile )
{
	...
	if (pDefines == NULL && m_VertexShader != 0)
		return true;
